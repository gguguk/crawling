...
for(region in regions)
{
  ...
  while(TRUE) # 페이지를 순회하며 크롤링 시작.
  {
    ...
    for(i in 1:length(context_urls)) # context_urls를 바탕으로 기사 내용 크롤링
    {
      ...
      } else # 가장 흔한경우. 첫번째 p태그엔 뉴스내용, 2번째 p태그에는 기자 이름, 첫번째 p 태그에 내용 + 두번째 p태그도 내용... 따라서 그냥 기자내용까지 다 긁어온다음에 나중에 정규표현식을 이용하여 기자이름을 삭제하는게 나을 것 같음.
      {
        cur_context = context_checker %>% html_text(trim=T) %>% str_replace_all("(\\n)+ {0,}", " ") %>% paste0(collapse=" ")
      }
